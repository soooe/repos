...
public class TestCharacterEnumerator {
	...
	private void check(final String inputStr, final String... expected) {
		assertThat(CharacterEnumerator.enumerateCombination(inputStr), is(containsInAnyOrder(expected)));
	}
	...
	public void inputHasThreeCharacter() {
		...
		check("aab", "aab", "aba", "baa");

		/* 全て異なる文字のとき */

		check("abc", "abc", "acb", "bac", "bca", "cab", "cba");
	...
	public void sizeOfOutputList() {
		...
		assertThat(factorial(inputStr.length()), is(equalTo(retBuffer.size())));
	}
